...
def one_hot(indices, depth, device):
    # Create a zero tensor with shape [batch_size, depth]
    encoded_indices = torch.zeros(indices.size(0), depth, device=device)
    
    # Place a 1 in the position of each index
    encoded_indices.scatter_(1, indices.unsqueeze(1), 1)
    
    return encoded_indices


def CosineNetHead(query, support, support_labels, n_way, n_shot, device, normalize=True):
    query = query.to(device).view(3, 20, -1)  # 确保形状为 [3, 20, 128]
    support = support.to(device).view(n_way, n_shot, -1)
    
    support_labels = torch.tensor(support_labels, dtype=torch.long, device=device)
    one_hot_labels = one_hot(support_labels, n_way, device).view(n_way, n_shot, n_way)
    
    # 计算每个类的原型
    prototypes = torch.sum(one_hot_labels * support, dim=1) / torch.sum(one_hot_labels, dim=1)
    prototypes = prototypes.view(n_way, -1)  # 改变形状为 [3, 128]
    ...
    # 正确调整形状以匹配
    query = query.unsqueeze(2)  # [3, 20, 1, 128]
    prototypes = prototypes.unsqueeze(1)  # [3, 1, 128]
    
    logits = F.cosine_similarity(query, prototypes, dim=-1)  # 应该是 [3, 20]
...
def meta_test():
    ...
    for i in range(10):
        ...
        test_f1 = f1_score(true.numpy(), preds.numpy(),average = 'weighted')
        test_nmi = normalized_mutual_info_score(true.numpy(), preds.numpy())
        nmi+=test_nmi
        precision, recall, _, _ = precision_recall_fscore_support(true.numpy(), preds.numpy(), average='weighted')
...
if __name__ == '__main__':
    ...
    with open('results_NFCSE_PCW.txt', 'w') as file:
