...
def meta_train():
    ...
    for epoch in range(100):
        ...
        a=0
        b=0
        for idx, (v, q_attack) in enumerate(zip(out_vectors, true)):
            # 从support_dist获取与v的攻击类型q_attack相同的嵌入列表
            vec_list_for_v = support_dist[q_attack]
            a=len(vec_list_for_v)
            different_type_vectors = []
            # 遍历support_dist，收集不同攻击类型的嵌入
            for label, vec_list in support_dist.items():
                if label != q_attack:  # 排除与v的攻击类型相同的嵌入
                    different_type_vectors.extend(vec_list)
                b=len(different_type_vectors)

            # 此时，different_type_vectors 包含了所有与v攻击类型不同的嵌入
            # 将嵌入列表转换为张量
            tensor_for_v = torch.stack(vec_list_for_v)  # 将列表转换为张量
            tensor_different_types = torch.stack(different_type_vectors)
            v = v.unsqueeze(0)  # 将 v 从 [C] 转换为 [1, C]
            # 计算 v 与同类型嵌入的相似度
            sim_same_type = torch.einsum('nc,ck->nk', [v, tensor_for_v.T])

            # 计算 v 与不同类型嵌入的相似度
            sim_different_types = torch.einsum('nc,ck->nk', [v, tensor_different_types.T])

            logits = torch.cat([sim_same_type, sim_different_types], dim=1)

            T=1.5
            logits/=T

            # 正规化处理，此处示例简化处理（实际使用中可能需要自定义正规化函数）
            max_val, _ = torch.max(logits, dim=1, keepdim=True)
            normalized_logits = logits - max_val  # 减去最大值以防止指数溢出

            # 应用softmax到正规化的logits
            softmax_output = F.softmax(normalized_logits, dim=1)

            # 将softmax结果添加到result_list，转换为Python list存储
            result_list.append(softmax_output.tolist())

            # 获取softmax输出中最可能的类别索引，并添加到preds列表
            batch_preds = softmax_output.argmax(dim=1).tolist()  # 获取整个批次的预测
            preds.extend(batch_preds)  # 添加到全局列表
            ...
            # 使用labels来进行后续的损失计算或模型训练
            logits0_list.append(logits)
            labels0_list.append(labels)
        logits_tensor = torch.stack(logits0_list)  # 将列表转换为形状为 [N, C] 的张量

        labels_tensor = torch.tensor(true, dtype=torch.long)  # 确保数据类型为 long
        ...
        f1+= test_f1
        if epoch % 10 == 0:
            print(f'Epoch {epoch}, Train F1: {test_f1:.4f}, Loss: {loss.item():.4f}')
    return f1/100
...
def meta_test():
    ...
    for i in range(10):
        ...
        nmi+= test_nmi
        precision, recall, _, _ = precision_recall_fscore_support(true, preds, average='weighted', zero_division=0)
...
if __name__ == '__main__':
    ...
    with open('results_NFCSE_CLSA.txt', 'w') as file:
