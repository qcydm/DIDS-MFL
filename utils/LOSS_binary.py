...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        class_mask.scatter_(1, ids.long().data, 1.)

        if inputs.is_cuda and not self.alpha.is_cuda:
            self.alpha = self.alpha.cuda()
        alpha = self.alpha[ids.long().data.view(-1)]
...
class Loss(torch.nn.Module):
    ...
    # 正常损失    
    def forward(self,bin_inputs,label, attack, z):
        # return self.Loss1(bin_inputs, label)
        return self.Loss1(bin_inputs, label) + self.Loss3(z)
